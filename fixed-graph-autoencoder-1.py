...
class GraphAutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for b in range(batch_size):
            ...
            # FIX 1: Apply normalization to prevent collapse
            # This helps maintain diversity in the latent space by ensuring points are spread out
            latent = latent * 3.0  # Scale latent space to be more spread out
...
def graph_l1_loss(latent_batch, edge_index_list):
    ...
    for b in range(batch_size):
        ...
        if edge_index.size(1) > 0:
            ...
            # We want a balance: not too far apart (graph_loss) but also not too close (prevent collapse)
...
# FIX 3: Add variance loss to prevent latent space collapse
def variance_loss(latent_batch):
    """Encourages latent points to have high variance (prevents collapse)"""
    batch_size = latent_batch.shape[0]
    total_loss = torch.tensor(0.0, device=latent_batch.device, requires_grad=True)
...
def train_model(model, dataloader, test_samples, epochs=100, lr=0.001, graph_weight=0.1, var_weight=0.5, save_every=10):
    ...
    # FIX 6: Use cosine annealing scheduler for better convergence
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch,) in enumerate(dataloader):
            ...
            # FIX 7: Balance the losses - adjust weights
            # Reduced graph_weight and added variance loss to prevent collapse
            total_loss = rec_loss + graph_weight * g_loss + var_weight * v_loss
            
            # Backward and optimize
            optimizer.zero_grad()
            total_loss.backward()
            
            # FIX 8: Add gradient clipping to prevent instability
...
def main():
    ...
    trained_model = train_model(
        ...
        epochs=200,
        lr=0.002,
        graph_weight=0.05,  # Reduced from 0.2 to prevent collapse
        var_weight=0.5,     # Added variance loss weight
        save_every=10
    )
