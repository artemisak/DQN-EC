...
from modules.utils import shift_graph, prepare_hypergraph, prepare_listener, prepare_speaker, prepare_combined_observations
...
class Args:
    ...
    wandb_project_name: str = "combined-true-learn"
    """the wandb's project name"""
    wandb_entity: Optional[str] = "ch4t5ky-itmo-university"
...
def process_combined_observations(env, speaker_gae, listener_gae, observations, speaker_agent, listener_agent, vectorizer, device):
    obs = prepare_combined_observations(observations[listener_agent], observations[speaker_agent], vectorizer)
    speaker_obs = obs[:11].unsqueeze(0).to(device)
    listener_obs = obs[11:].unsqueeze(0).to(device)
    # absolute positions (world coordinates)
    speaker_pos = env.unwrapped.world.agents[0].state.p_pos
    listener_pos = env.unwrapped.world.agents[1].state.p_pos

    with (torch.no_grad()):
        _, _, graphs = listener_gae(speaker_obs)
        ls_graph = shift_graph(graphs[0], x=listener_pos[0], y=listener_pos[1])

        _, _, graphs = speaker_gae(listener_obs)
        sp_graph = shift_graph(graphs[0], x=speaker_pos[0], y=speaker_pos[1])
...
def main():
    ...
    listener_gae.load_state_dict(torch.load('checkpoints/combined_gae.pt', map_location=device))
    listener_gae.eval()
    ...
    speaker_gae.load_state_dict(torch.load('checkpoints/combined_gae.pt', map_location=device))
    speaker_gae.eval()
    ...
    #                                                         filter,
    #                                                         device)
    current_hypergraph = process_combined_observations(env, speaker_gae, listener_gae, observations, speaker_agent, listener_agent, vectorizer, device)
    for global_step in range(args.total_timesteps):
        ...
        #                                                      filter,
        #                                                      device)
        next_hypergraph = process_combined_observations(env, speaker_gae, listener_gae, observations, speaker_agent, listener_agent, vectorizer, device)
        ...
        if any(terminations.values()) or any(truncations.values()):
            ...
            if args.track and episode_count % 2 == 0:
                # Log to W&B
                wandb.log({
