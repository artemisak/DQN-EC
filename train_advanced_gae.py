...
def train(model, generator, epochs, lr):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch,) in enumerate(generator.loader):
            ...
            predicted_logits, predicted_values, graphs = model(generator.combined[batch])
            ...
            first_term, second_term, recon_loss = reconstruction_loss(
                generator.combined[batch][:, :, :4], predicted_logits,
                generator.combined[batch][:, :, 4:], predicted_values,
                w1, w2
            )
...
if __name__ == "__main__":
    ...
    generator = SyntheticData(use_combined=True)
    ...
    torch.save(model.state_dict(), "checkpoints/combined_gae.pt")
